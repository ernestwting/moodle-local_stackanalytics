...
/**
 * PDF export for the dashboard, built on Moodle core's own bundled TCPDF
 * wrapper (lib/pdflib.php's \pdf class) rather than vendoring a separate
 * copy — core has shipped TCPDF for its own PDF-generating features
 * (badges, certificates, mod_assign feedback) for years, confirmed against
 * a real Moodle 4.5 core checkout, so there's no need to duplicate that
 * ~5MB third-party library into this plugin the way local_quizanalytics's
 * vendored copy does (that plugin predates checking for this).
 *
...
namespace local_stackanalytics\analytics;
 ...
 */
class stackanalytics_pdf extends \pdf {
