 ...
 * bytes via stackanalytics_pdf (core's bundled TCPDF, see that class's
...
class pdf_builder {
    ...
    public static function build(string $coursename, array $sections): string {
        $pdf = new stackanalytics_pdf('L', 'mm', 'A4', true, 'UTF-8');
     ...
     *
     * @param stackanalytics_pdf $pdf
     * @param array $section {title, columns, rows, shown, total, truncated}
     * @param bool $isfirst
     */
    private static function render_section(stackanalytics_pdf $pdf, array $section, bool $isfirst): void {
        if (!$isfirst) {
     ...
     * @param array[] $rows
     */
    private static function render_table(stackanalytics_pdf $pdf, array $columns, array $rows): void {
